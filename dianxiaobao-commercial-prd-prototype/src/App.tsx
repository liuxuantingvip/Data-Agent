        ...
        <section className="prd-info-panel">
          ...
          <PrdSection title="需求背景">
            <p className="prd-one-line">
              钉钉 AI 表格里已有店小宝数据源插件，用户已经能产生明确的电商数据取数需求，但当前还没有把试用到期、会员购买、平台选择、支付开通和继续取数串成自助商业化链路。
            </p>
          </PrdSection>
          <PrdSection title="目标">
            <p className="prd-one-line">
              在不推翻现有钉钉数据源插件的前提下，完成会员商业化和前台体验改版，让用户在需要取数时可自助购买、选择可用平台、支付开通并回到当前链路继续取数。
            </p>
          </PrdSection>
          <PrdSection title="用户故事">
            {renderTable([
              ["角色", "用户故事"],
              ["运营", "我需要把现有钉钉数据源的取数需求转成可售卖、可开通、可追踪的会员商业化链路，而不是依赖人工判断和开通。"],
              ["用户", "我在钉钉 AI 表格里使用数据源取数时，如果试用到期或没有可用权益，希望直接完成购买并回到当前配置继续取数。"],
              ["运营", "我需要在订单和租户里看到支付结果、已选平台和权益发放结果，发放失败时可以定位原因并重试。"],
            ])}
          </PrdSection>
